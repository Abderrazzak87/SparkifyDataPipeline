...
class LoadDimensionOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        if not self.append_only:
            self.log.info("Delete befor Insert the dimension table {}".format(self.table))
            redshift.run("DELETE FROM {}".format(self.table))        
        self.log.info("Insert data from staging tables into the dimension table {}".format(self.table))
